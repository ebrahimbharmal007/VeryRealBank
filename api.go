...
func (s *APIServer) Run() {
	...
	router.HandleFunc("/account/{accnum}", makeHTTPHandleFunc(s.handleUpdateAccount)).Methods("PUT")
...
func (s *APIServer) handleUpdateAccount(w http.ResponseWriter, r *http.Request) error {
	...
	err := validate.Struct(updateAccountReq)
	if err != nil {
		log.Printf("\nValidation error: %+v\n", err)
	...
	account, err := s.service.UpdateAccount(*updateAccountReq, int64(id))
